                ...
                <form class="form-horizontal" method="post" action="{{url('/product/'.$product->id.'/store/winkel')}}">
                    @else
                        <form class="form-horizontal" method="post"
                              action="{{url('/product/'.$product->getCartIdByProductUserId($product->id, auth()->user()->id).'/update/winkel')}}">
                            @endif
                            @csrf
                            <div style="text-align: center;" id="winkelDiv" class="row">
                                <div id="productAfbeeldingDiv" class="col-xs-12 col-sm-6 col-lg-3">
                                    <img class="productAfbeelding" src="/storage/cover_images/{{$product->cover_image}}"
                                         alt="Product">
                                </div>
                                ...
                                <div style="margin-top: 40px;" class="col-xs-12 col-sm-6 col-lg-2">
                                    <select style="width: 100%; padding-top: 6px; padding-bottom: 6px;"
                                            id="voorraadSelectbox" class="tabelWinkel" name="aantal">
                                ...
                                <div style="margin-top: 40px;" class="col-xs-12 col-sm-6 col-lg-2">
                                    <button style="width: 100%;" id="winkelwagenButton" class="tabelWinkel btn"
                                            type="submit" name="winkelwagen">Winkelwagen
                                    </button>
                                </div>
                            </div>
                            <hr>
